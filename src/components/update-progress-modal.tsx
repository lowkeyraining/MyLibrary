...
export function UpdateProgressModal({ bookId, title, author, totalPages, initialPage }: UpdateProgressModalProps) {
  ...
  const handleSave = async () => {
    ...
    if (result.success) {
      setIsOpen(false)
      setNote("")
      router.refresh() // ← refresh เพื่ออัปเดต status badge อัตโนมัติ
    } else {
  ...
  const setFinish = () => { if (totalPages > 0) setPage(totalPages) }
      ...
      <DialogContent className="sm:max-w-[400px] p-0 bg-white border-none shadow-[0_24px_60px_rgba(92,64,51,0.18)] rounded-2xl overflow-hidden [&>button]:hidden">
        ...
        <div className="px-6 py-5 border-b border-[#D9D2C7]">
          ...
          <div className="relative h-[10px] bg-[#EDE8DF] rounded-full mb-3 w-full group">
            <div className="absolute left-0 top-0 h-full bg-gradient-to-r from-[#C07B5A] to-[#D08A6A] rounded-full pointer-events-none" style={{ width: `${pct}%` }}>
            ...
            <input type="range" min={0} max={totalPages > 0 ? totalPages : 100} value={page}
              onChange={handlePageChange} disabled={totalPages <= 0}
        ...
        <div className="px-6 py-5">
          <div className="mb-4">
            ...
            <div className="flex items-center gap-2">
              <input type="number" value={page || ""} onChange={handlePageChange} min={0} max={totalPages || undefined}
          ...
          <div className="flex flex-wrap gap-1.5 mb-4">
            ...
            {[10, 25, 50].map(n => (
              <button key={n} onClick={() => addPages(n)}
                className="px-2.5 py-1 bg-[#EDE8DF] border border-[#D9D2C7] rounded-md text-[11px] font-semibold text-[#8B6F5E] hover:border-[#C4A882] hover:text-[#5C4033] hover:bg-[#C4A882]/10 transition-colors">
                +{n}
              </button>
            ))}
            <button onClick={setFinish}
              className="px-2.5 py-1 bg-[#EDE8DF] border border-[#D9D2C7] rounded-md text-[11px] font-semibold text-[#8B6F5E] hover:border-[#C4A882] hover:text-[#5C4033] hover:bg-[#C4A882]/10 transition-colors">
              จบแล้ว
            </button>
          </div>
          ...
          <div className={`flex items-center gap-2 text-[12px] text-[#9CAF88] font-medium transition-all duration-300 ${page >= totalPages && totalPages > 0 ? 'opacity-100 mb-4 h-auto' : 'opacity-0 h-0 overflow-hidden mb-0'}`}>
            <Check size={14} strokeWidth={2.5} /> ยินดีด้วย! คุณอ่านครบแล้ว
          </div>
        ...
        <div className="px-6 py-4 border-t border-[#D9D2C7] flex justify-end gap-2.5">
          ...
          <button onClick={handleSave} disabled={isLoading}
            className="px-5 py-2 bg-[#5C4033] text-white border-none rounded-lg text-[13px] font-semibold hover:bg-[#4A332A] transition-colors flex items-center gap-1.5 disabled:opacity-50">
