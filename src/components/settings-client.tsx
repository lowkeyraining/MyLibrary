...
export function EditProfileModal({ user, children }: { user: any, children: React.ReactNode }) {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          // แปลงเป็น Base64 โดยลดคุณภาพลงเหลือ 70% (เพื่อให้ไฟล์เล็กมาก)
  ...
  const handleSave = async () => {
    ...
    if (res.success) {
      setIsSuccess(true)
      setPassword("") 
      // ปิดป๊อปอัปอัตโนมัติเมื่อบันทึกเสร็จ
      ...
      <DialogContent className="sm:max-w-[425px] bg-[#FAF9F6] border-[#D9D2C7] p-6 rounded-2xl">
        ...
        <div className="flex flex-col gap-5 mt-2">
          ...
          <div className="flex flex-col gap-3.5">
            ...
            <div>
              <label className="text-[12px] font-semibold text-[#5C4033] mb-1.5 block">รหัสผ่านใหม่ <span className="font-normal text-[#8B6F5E]">(เว้นว่างหากไม่เปลี่ยน)</span></label>
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[400px] bg-[#FAF9F6] border-[#D9D2C7] p-6 rounded-2xl">
        ...
        <div className="flex items-center gap-3 mb-4 mt-2">
           <input 
             type="number" value={target || ""} onChange={(e) => setTarget(parseInt(e.target.value) || 0)} min={1}
