...
export default async function DashboardPage() {
  ...
  const [
    ...
    prisma.book.count({ where: { userId, status: "WANT_TO_READ" } }),
    ...
    prisma.readingGoal.findFirst({
      where: { userId, period: "YEARLY", year: currentYear },
    }),
    prisma.book.findMany({
      where: {
        userId,
  ...
  const trendData = Array.from({ length: 12 }, (_, i) => {
    ...
    const total = completedWithDate.filter((b) => {
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8 fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-5">
          <GoalCard
            year={currentYear}
            target={yearlyGoal?.targetBooks ?? null}
            completed={completed}
          />
        </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          ...
          <Link href="/books" className="text-xs text-[#C07B5A] hover:underline">
            View all &#x2192;
          </Link>
        </div>
